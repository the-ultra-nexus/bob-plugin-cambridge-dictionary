...
- **Separator**: not a separate addition. The 60 `=` separator is **embedded inside the previous addition's value** with `\n\n` prefix (e.g. `"...\n\n==============================..."`). Reason: Bob trims leading/trailing whitespace on addition values, so a standalone `name=""` SEPARATOR cannot produce a blank line above. By embedding the `\n\n` + `====` inside the previous block, the internal `\n`s render as a visible blank line. Last summary block embeds the separator (between summary and first detail section). Each non-last detail block embeds the separator (between detail sections).
- **POS detailed sections**: one entry per POS, `name = <pure POS label>`, `value` = all definitions (`level+grammar` → EN → `> CN` → `    • examples`), phrase panels (`(title)` → EN → `> CN` → `    • examples`), then 习语/短语动词 with `① ② …` numbered items inline. Every def-block ends with a blank line (including phrase panels). All but the last detail block end with `\n\n` + 60 `=` (the embedded separator).
